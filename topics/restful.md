...
> 
>
> **TODO**: Regeln für fehlende HTTP Methods 
>
> 
...
The technical conception of pagination should also consider user experience related issues. As mentioned in this [article](https://www.smashingmagazine.com/2016/03/pagination-infinite-scrolling-load-more-buttons/), jumping to a specific page is far less used than navigation via `next`/`prev` page links (See [**SHOULD** use pagination links where applicable](#should-use-pagination-links-where-applicable)).

**Note:** To provide a consistent look and feel of pagination patterns, you must stick to the common query parameter names defined in [**MUST** stick to conventional query parameters](#must-stick-to-conventional-query-parameters).

### **[SHOULD]** use pagination links where applicable

> TODO: [REVIEW]

To simplify client design, APIs should support hypertext controls for pagination over collections whenever applicable. Beside `next` this may comprise the support for `prev`, `first`, `last`, and `self` as [link relations](http://www.iana.org/assignments/link-relations) (see also [link-relation types](#link-relation-types) for details).

The page content is transported via `item` links and/or with `_embedded` `item` objects. The format for pagination links **[MUST]** conform to the HAL format.
