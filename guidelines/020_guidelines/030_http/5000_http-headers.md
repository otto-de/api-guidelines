...
| `ETag`                  | Used for [caching](/guidelines/030_http/5010_may-use-etag-header-for-caching-resources.md) and [concurrency control](5030_may-use-etag-together-with-if-match-if-none-match-header-for-concurrrency-control.md). The entity tag value must be a double quoted string to comply with [RFC 7232](https://tools.ietf.org/html/rfc7232#section-2.3): `ETag: "foo-bar"` or `ETag: W/"foo-bar"` indicating a "weak" tag. | ✗               | ✔                |
...
| `Location`              | Contains an URL to redirect to. This URL must be fully qualified. The header is either set for redirection (`3xx`) or resource creation. When answering via `201 Created` following a `POST` request, you should give the resource URL in the `Location` header. Resources created via `PUT` should not set a `Location` header, as the resource URL is known to the client prior to creation.                     | ✗               | ✔                |
