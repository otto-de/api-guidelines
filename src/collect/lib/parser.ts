...
export class Parser {
  ...
  constructor(parser: MarkdownIt, config: Config, data: string, level: number, source: string) {
    this.source = source;
    this.level = level;
    this.data = data;
    this.parser = parser;
  ...
  public remapLink(token: Token, parent?: Token): void {
    ...
    if (matches?.groups?.hash) {
      ...
      let path = normalize((isAbsolute(matches.groups.path) ? "." : "") + matches.groups.path);
      ...
      const target = [...Parser.sourceMap.keys()].find((k) => k.indexOf(path) > -1);
      ...
      if (target) {
        ...
        token.attrSet(
          "onClick",
          ["alert('This link is broken!", `href: ${href}`, `Source${this.source}`, "')"].join("\\n")
        );
  ...
  public checkAndProcessRule(): void {
    if (Parser.isRule(this.frontMatter)) {
      ...
      const match = new RegExp(this.config.rules.matcher, "m").exec(this.frontMatter.id);
  ...
  public processHeadings(): ProcessedHeading[] {
    ...
    for (let i = 0, len = this.tokens.length; i < len; ) {
      ...
      if (
        ...
        inline.children = this.parser.parseInline(format(aFormat, id, markup), {})[0].children;
