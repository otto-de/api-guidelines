...
export async function collectCategory(
  ...
  const docs = await Promise.all(
    docsPaths.map(async (p) => new Parser(parser, config, await readText(p), nextLevel, p))
  );
...
export async function collectCategorys(
  ...
  const data = await Promise.all(dirs.map((d) => collectCategory(d, parser, config, level)));
